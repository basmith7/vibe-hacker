# Ideas — staging inbox

A holding pen for ideas that aren't ready to be formalized yet. This is **not** a second backlog —
it's a staging area. Things move *through* it and then leave it.

## How this works

- **💡 Ideas** — the user's thinking space. The agent may add input as indented sub-bullets
  under an idea (questions, tradeoffs, prior art in the code), but must **not** action, formalize, or
  move these. They're a conversation, not a work order.
- **📥 Ready** — the user has blessed these; they are now the agent's to route out. Once routed, the
  item is **deleted from this file** (this file must not accumulate — if it grows into a mirror of
  todo.md, the workflow has failed).

**Routing has two destinations:**

- Small, self-contained ideas → a line in [`../todo.md`](../todo.md).
- Large, multi-phase efforts → their own phased design doc in this folder (like
  [`window-manager.md`](window-manager.md) / [`crafting-update.md`](crafting-update.md)), then a
  pointer line in `todo.md` if useful.

The agent picks the destination based on size; when in doubt, ask.

---

## 💡 Ideas

*(User's space. Agent comments inline as sub-bullets; does not action or move.)*

* Walk me though the code. I'd like to understand the architecture better.
* set my computer to not sleep when claude is working
* "slack" as a purchasable item, should have insane messages about really dumb sales people storeies, and project managers saying "nothing from my end" and it should hurt productivity while its open.  

## 📥 Ready

*(User has blessed these. Agent formalizes and routes each out, then deletes it from here.)*

_(nothing pending — batch routed 2026-07-07: espresso/assist→gear and achievements-as-app → todo.md;
title-bar/taskbar merge + credits/clock bottom-right → window-manager.md queued. Two were already
shipped: "start menu only shows purchased apps" = playtest-batch-5's apps-only Start Menu; "better
default window layout" = playtest-batch-6's non-overlapping grid.)_
